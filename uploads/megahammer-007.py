...
# Version 2.2.0  - Added 'Add Leak Sensor' option
#
################################################################################
# Make an EXE:
...
def firmwareupdate():
    ...
    printbreak()
    print ("""
    Please connect to the same Wi-Fi network
    as the Valve Controller
    """)
    raw_input("Press Enter to continue")
    printbreak()
    print "Searching subnet for Valve Controller, please wait..."
    if (ans=="Y" or ans=="y"): 
        beta = True
    elif (ans=="N" or ans=="n"):
        beta = False
...
def pairleak():
    print ("""
        Please enter the 4 character PIN from the bottom of the sensor
        """)
    pin = raw_input(" >>> ").upper()
    printbreak()
    print "Sending request..."
    guardian = str(hammertime())
    message = '{"command":"register_mode", "mode":7, "PIN":"'+ pin +'","type":0}'
    senditudp(message, guardian)
    print "DONE!!"
    printbreak()
    print "SHAKE THAT LEAK SENSOR! MAKE IT BEEP!"
    time.sleep(3)
